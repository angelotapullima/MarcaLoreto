...
class LicenseesBloc {
  ...
  final _businessCategoryController =
      BehaviorSubject<List<BusinessCategoryModel>>();
  Stream<List<BusinessCategoryModel>> get businessCategoryStream =>
      _businessCategoryController.stream;
  ...
  Stream<List<BusinessModel>> get businessOnlyStream =>
      _businessOnlyController.stream;
  ...
  void getBusinessCategory() async {
    _businessCategoryController.sink
        .add(await _api.businessCategoryDB.getBusinessCategory());
    await _api.getInit();
    _businessCategoryController.sink
        .add(await _api.businessCategoryDB.getBusinessCategory());
  }
  ...
  void getBusiness(String idBusiness) async {
    _businessOnlyController.sink
        .add(await _api.businessDb.getBusinessByIdBusiness(idBusiness));
    await _api.getInit();
    _businessOnlyController.sink
        .add(await _api.businessDb.getBusinessByIdBusiness(idBusiness));
  }
  ...
  Future<List<BusinessModel>> getBusinessList(String idBusinessCategory) async {
    ...
    final _listBusiness =
        await _api.businessDb.getBusinessByIdCategory(idBusinessCategory);
    ...
    for (var i = 0; i < _listBusiness.length; i++) {
      ...
      business.stateBusiness = _listBusiness[i].stateBusiness;
      business.activateEnglish = _listBusiness[i].activateEnglish;

      final _categoria = await _api.businessCategoryDB
          .getBusinessCategoryById(idBusinessCategory);
