...
class BlogDatabase {
  ...
  Future<List<BlogModel>> getBlogs() async {
    try {
      ...
      List<BlogModel> list = [];
      List<Map> maps =
          await db.rawQuery("SELECT * FROM Blogs WHERE estadoBlog='1' ");
  ...
  Future<List<BlogModel>> getBlogById(String idBlog) async {
    try {
      ...
      List<BlogModel> list = [];
      List<Map> maps =
          await db.rawQuery("SELECT * FROM Blogs WHERE idBlog='$idBlog' ");
  ...
  updateLanguage(String value) async {
    try {
      ...
      final res =
          await db.rawUpdate("UPDATE Blogs SET activarEnglish='$value'");
